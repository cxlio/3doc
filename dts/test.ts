...
export default spec('dts', a => {
	...
	a.test('JSOoc comments', (a: TestApi) => {
		...
		a.assert(p1?.docs?.content?.[0]);
		a.assert(p2?.docs?.content?.[0]);
...
export interface ABC {};
	...
	a.test('type alias', a => {
		...
		a.test('type parameters', (a: TestApi) => {
			const type = I;
			a.assert(type?.id);
...
function map<T>() {	return operator<T>(); }
	...
	a.test('object literal infered type', (a: TestApi) => {
		...
		a.assert(one && two && three && four && five);
...
`);
	...
	a.test('Resolved Type', a => {
		...
		a.test('Variable Type', (a: TestApi) => {
			a.assert(E?.type?.type?.resolvedType);
			a.equal(E.type.type.resolvedType.children?.length, 3);
		});
	});
